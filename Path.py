...
class Path(object):
	...
	# ==============================================================================================================
        ...
        # run concorde and stream output; show a simple progress indicator (lines processed)
        proc = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, bufsize=1)
        pbar = tqdm(desc='concorde', unit='lines')
        try:
        ...
        if proc.returncode != 0:
            raise RuntimeError('concorde failed with return code %d' % proc.returncode)
        ...
        tmp.remove('')
        nds = np.array(list(map(int, tmp)), dtype=int)
